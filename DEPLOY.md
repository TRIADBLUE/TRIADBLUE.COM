...
- `bb-url.png`, `coachblue-icon.png`, `builderblue2-url.png` — the three NEW
  brand logos (YOU add these three files — see step 1). All other images load
  from the CDN at https://cdn.triadblue.com/brands/... automatically.
- `package.json` — tells Railway how to serve the site

## Steps

1. **Upload these files to the GitHub repo** (TRIADBLUE/triadblueinc).
   On the repo page: Add file → Upload files → drag ALL files in flat (no
   folders needed), PLUS your three logo PNGs renamed EXACTLY: bb-url.png, coachblue-icon.png, builderblue2-url.png → Commit.
